...
def analyze_relation_types(relations_csv_path: str) -> dict:
    """Analyze relation type distribution from a relations CSV file.
    ...
    Returns:
        Dict with keys:
            total (int): Total number of relations.
            by_type (dict[str, int]): Count per relation type.
            concepts_with_non_prereq (set[int]): FromID values with non-prerequisite relations.
    """
    by_type: Dict[str, int] = defaultdict(int)
    concepts_with_non_prereq: Set[int] = set()
    ...
    with open(relations_csv_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            if rel_type != 'prerequisite':
                concepts_with_non_prereq.add(int(row['FromID']))

    return {
        'total': total,
        'by_type': dict(by_type),
        'concepts_with_non_prereq': concepts_with_non_prereq,
    }
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python analyze-graph.py <input_csv> <output_report.md> [--relations relations.csv]")
        print("\nExample:")
        print("  python analyze-graph.py learning-graph.csv quality-metrics.md")
        print("  python analyze-graph.py learning-graph.csv quality-metrics.md --relations relations.csv")
        sys.exit(1)

    csv_path = sys.argv[1]
    output_path = sys.argv[2]

    # Check for --relations flag
    relations_csv = None
    if '--relations' in sys.argv:
        idx = sys.argv.index('--relations')
        if idx + 1 < len(sys.argv):
            relations_csv = sys.argv[idx + 1]

    generate_report(csv_path, output_path)

    if relations_csv:
        result = analyze_relation_types(relations_csv)
        # Append to report
        with open(output_path, 'a', encoding='utf-8') as f:
            f.write("\n## Relation Type Distribution\n\n")
            f.write(f"- **Total typed relations**: {result['total']}\n\n")
            f.write("| Type | Count |\n|------|-------|\n")
            for rtype, count in sorted(result['by_type'].items()):
