...
VALID_RELATION_TYPES = {'prerequisite', 'related_to', 'contrasts_with', 'is_part_of'}


def load_relations(relations_csv_path: str) -> List[dict]:
    """Load relations.csv -> list of {from: int, to: int, type: str}.

    Args:
        relations_csv_path: Path to CSV with columns FromID, ToID, RelationType.

    Returns:
        List of dicts with 'from' (int), 'to' (int), 'type' (str) keys.
    ...
    with open(relations_csv_path, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            rtype = row['RelationType']
            if rtype not in VALID_RELATION_TYPES:
                print(f"⚠️  WARNING: Unknown relation type '{rtype}' in row: FromID={row['FromID']}, ToID={row['ToID']}. Skipping.")
                continue
            relations.append({
                'from': int(row['FromID']),
                'to': int(row['ToID']),
...
def merge_relations_into_edges(edges: List[dict], relations: List[dict]) -> List[dict]:
    """Merge typed relations into edges.

    - Existing edges matching (from, to) get a 'type' field from the relation.
    - Relations not matching any existing edge create new edges.
    - Edges without a matching relation default to type='prerequisite'.

    Args:
        edges: List of edge dicts with 'from' and 'to' keys.
        relations: List of relation dicts with 'from', 'to', and 'type' keys.

    Returns:
        New list of edge dicts, each with a 'type' field.
    """
    # Build lookup from relations keyed by (from, to)
    rel_lookup = {(r['from'], r['to']): r['type'] for r in relations}

    # Track which relations are consumed by existing edges
    consumed = set()
    merged = []

    for edge in edges:
        key = (edge['from'], edge['to'])
        new_edge = {'from': edge['from'], 'to': edge['to']}
        if key in rel_lookup:
            new_edge['type'] = rel_lookup[key]
            consumed.add(key)
        else:
            new_edge['type'] = 'prerequisite'
        merged.append(new_edge)

    # Add relations that did not match any existing edge
    for r in relations:
        key = (r['from'], r['to'])
        if key not in consumed:
            merged.append({'from': r['from'], 'to': r['to'], 'type': r['type']})
...
def csv_to_json(csv_path: str, json_path: str, color_config: dict = None,
                ...
                relations_csv_path: Optional[str] = None):
    """
    ...
    Args:
        ...
        relations_csv_path: Optional path to relations.csv with columns FromID, ToID, RelationType.
                           When provided, edges gain a 'type' field; without it, output is unchanged.
    """
    ...
    # Optionally merge typed relations into edges
    if relations_csv_path is not None:
        relations = load_relations(relations_csv_path)
        edges = merge_relations_into_edges(edges, relations)
...
if __name__ == "__main__":
    ...
    relations_csv_path = None
    if len(sys.argv) > 6:
        relations_file = sys.argv[6]
        if os.path.exists(relations_file):
            relations_csv_path = relations_file
            print(f"Loaded relations from: {relations_file}")
